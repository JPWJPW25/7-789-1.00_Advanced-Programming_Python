...
#Import and retrieve libraries as well as functions used for file handling
import os
...
#Import and retrieve libraries as well as functions used to preprocess txt files 
import nltk
import string
...
#Import and retrieve libraries as well as functions used to compute similarity between txt files
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.metrics.pairwise import cosine_similarity

#Import and retrieve libraries as well as functions to construct data frame and lower triangle heatmap
import pandas as pd
import numpy as np
...
#Write two functions to convert from 1) PDF to txt and 2) Word to txt

#Function 1) to convert PDF files into txt files
...
#Function 2) to convert Word files into txt files
...
#Write two functions to 1) retrieve file information (filepath and -name) and 2) to retrieve content
...
#Write five subfunctions to 1) tokenize the content, 2) remove stopwords, 3) remove punctuations, 4) convert to lower case,
#and 5) apply stemming algorithm. Finally include all subfunctions in 6) one main preprocessing function.
...
#Function 3: Remove punctuation 
#(i.e. .,!?) and add individual items to remove
def remove_Punctuation(filteredContents_a):
    ...
    #Due to conversion of text with pdfplumber too many line breaks have been included
    specialsigns = "\\n"
    exclude_Puncuation_set.add(specialsigns)
...
while restartProgram == "Yes": 
    ...
    # Step 11: Include breakpoint to view dataframe in Visual Studio code #
    #Include a breakpoint here, so the dataframe can be viewed in Data Viewer of Visual Studio code 
    df
    #Alternative when viewing code in Jupyter Notebook - remove "#"
    #display(df.style)
